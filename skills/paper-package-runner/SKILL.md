...
Core principle: ask only for the truly blocking inputs, derive safe defaults for the rest, and route the run through the existing production skills instead of duplicating their logic.
...
Before asking the user anything beyond the PDF path and target language:
...
- Required user inputs: `paper_pdf_path`, `target_language`
...
| Mistake | Correction |
|--------|------------|
| Asking the user for every optional field up front | Ask only for `paper_pdf_path` and `target_language`; derive the rest unless overrides are needed. |
